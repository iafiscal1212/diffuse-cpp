...
% v2.0 — March 22, 2026
% Updated with inter-step KV cache results
%
...
with LLaDA-8B Q4\_K\_M, \texttt{diffuse-cpp} achieves 15--28~tok/s with entropy-exit
scheduling, inter-step KV cache, and 256-token generation on easy prompts---up to
$3.2\times$ faster than \texttt{llama.cpp} with Llama-3-8B Q4\_K\_M (8.51~tok/s) on the
same hardware. Thread scaling reaches $7.5\times$ from 1 to 12 cores for diffusion versus
saturation at 4 cores for AR. An entropy-based adaptive scheduler reduces denoising steps
from 16 to 2--4 on easy prompts without quality loss, providing up to $6\times$ speedup
over the F16 baseline. An inter-step KV cache reuses Key and Value tensors between
denoising steps, providing $1.6\times$ average speedup with no quality degradation and
implicit regularization benefits. Increasing the generation buffer from 64 to 256 tokens
yields 20\% lower cost per token, confirming that diffusion's matrix-matrix advantage
amplifies with sequence length. These results suggest that diffusion models may be a
viable path toward democratizing LLM inference on commodity hardware without GPU dependency.
\end{abstract}

\textbf{Keywords:} diffusion language models, CPU inference, memory-bound, compute-bound,
LLM acceleration, quantization, adaptive scheduling, inter-step cache

\textbf{License:} CC-BY-4.0 (paper), AGPL-3.0 (code)
...
\item \textbf{\texttt{diffuse-cpp}}: the first C++ inference engine for dLLMs, built on
  GGML~\cite{gerganov2023}, with support for F16, Q8\_0, and Q4\_K\_M quantization.
...
\item \textbf{Systematic benchmarking}: 288 configurations with 3 repetitions each,
  plus 8 diverse real prompts, totaling 6.5 hours of continuous CPU measurement.
\end{enumerate}
...
Llama-style backbone. SEDD~\cite{lou2024} proposed score-based discrete diffusion.
MDLM~\cite{sahoo2024} simplified the training objective. Prophet~\cite{li2025}
introduced speculative decoding for discrete diffusion, using a confidence-gap criterion
for early stopping. EAGS~\cite{koh2024} and Fast-dLLM~\cite{wu2025} explored
entropy-adaptive scheduling. All prior work benchmarked on GPU.
...
\textbf{Forward pass.} The model implements a standard Llama transformer architecture
with one critical difference: attention is bidirectional (no causal mask). All token
positions attend to all others at every step. There is no KV cache in the standard
autoregressive sense; the full attention computation is performed at each denoising step.
While this appears wasteful relative to AR inference, it is precisely this structure that
enables the compute-bound regime: the large matrix-matrix multiplications at each step
...
SafeTensors via a Python script (\texttt{tools/convert-llada.py}). The converter maps
LLaDA's tensor names to GGML conventions and validates architecture parameters (hidden
size, number of heads, vocabulary size, RoPE configuration).

\textbf{Quantization.} \texttt{diffuse-cpp} supports three quantization levels via
\texttt{diffuse-quantize}: F16 (14.9~GB), Q8\_0 (8.4~GB), and Q4\_K\_M (5.1~GB).
Quantization is performed weight-only; activations remain in FP32. The Q4\_K\_M format
uses mixed 4-bit/6-bit precision with per-block scaling, following the GGML specification.
...
\textbf{Model quality.} LLaDA-8B produces noticeably lower-quality outputs than
Llama-3-8B. We observed repetitions (``the and and molecules''), incomplete outputs on
code prompts, and mediocre translations. The throughput comparison is between paradigms,
not quality-equivalent models. A fair comparison would require a diffusion model trained
to match Llama-3-8B quality, which does not yet exist.

\textbf{Different models.} We compare LLaDA-8B (diffusion) against Llama-3-8B
(autoregressive). These share the same parameter count and Llama backbone architecture
but differ in training procedure, data, and generation mechanism. The comparison
demonstrates the \emph{computational} regime inversion, not a head-to-head model quality
contest.
...
tokens, and support for additional architectures (SEDD, MDLM).
...
engine for diffusion LLMs. On an AMD EPYC 12-core CPU, thread scaling reaches
$7.4\times$ for diffusion versus $2.4\times$ for AR. With Q4\_K\_M quantization,
entropy-exit scheduling, inter-step KV cache, and 256-token generation,
\texttt{diffuse-cpp} achieves 15--28~tok/s on real prompts---up to $3.2\times$ faster
than \texttt{llama.cpp}'s 8.51~tok/s on the same hardware, with 6 of 8 prompts
outperforming the AR baseline. The inter-step cache provides $1.6\times$ average
speedup with no quality degradation. Increasing the generation buffer from 64 to 256
tokens reduces per-token cost by 20\%, confirming that diffusion's advantage amplifies
...
entropy-adaptive scheduling literature~\cite{koh2024,wu2025}. Figures~1--3 were
